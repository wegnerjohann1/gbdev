...
instruction instructions[0x100] =
    {
        ...
        [0x2A] = {IN_LD, AM_R_HLI, RT_HL, RT_A},
        ...
        [0x3A] = {IN_LD, AM_R_HLD, RT_HL, RT_A},
        ...
        // 0xE.
        [0xE0] = {IN_LD, AM_A8_R, RT_NONE, RT_A},
...
void inst_to_str(cpu_context *ctx, char *str)
{
    ...
    case AM_R_A8:
        sprintf(str, "%s %s (FF%02X)", inst_name(inst->type), rt_lookup[inst->reg_1], bus_read(ctx->regs.PC - 1));
        return;

    case AM_R_D8:
        sprintf(str, "%s %s $%02X", inst_name(inst->type), rt_lookup[inst->reg_1], ctx->fetched_data);
        return;
    ...
    case AM_R_A16:
        sprintf(str, "%s %s (%04X)", inst_name(inst->type), rt_lookup[inst->reg_1], bus_read16(ctx->regs.PC - 2));
        return;
    ...
    case AM_A16_R:
        sprintf(str, "%s (%04X) %s", inst_name(inst->type), bus_read16(ctx->regs.PC - 2), rt_lookup[inst->reg_2]);
        return;
    ...
    case AM_A8_R:
        sprintf(str, "%s ($FF%02X) %s", inst_name(inst->type), bus_read(ctx->regs.PC - 1), rt_lookup[inst->reg_2]);
        return;

    case AM_D8:
        sprintf(str, "%s $%02X", inst_name(inst->type), ctx->fetched_data);
        return;
