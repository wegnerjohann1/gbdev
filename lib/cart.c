...
const char *cart_lic_name()
{
    ...
    {
        u16 int_lic_code = strtol(ctx.header->new_lic_code, NULL, 16); //convert new_lic_code string to integer
        if (int_lic_code <= 0xA4)
            return NEW_LIC_CODE[int_lic_code];
    }
    else if (ctx.header->lic_code <= 0xFF)
...
bool cart_load(char *cart)
{
    ...
    printf("\t Checksum : %2.2X (%s)\n", ctx.header->checksum, (x == ctx.rom_data[0x14D]) ? "PASSED" : "FAILED");
...
{
    // TODO Implement rom bank switch
